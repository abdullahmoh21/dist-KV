...
enum RS_STATUS rs_get(RedisStore *store, BulkString *key_str, RedisObject **out){
...
enum RS_STATUS rs_zadd(RedisStore *store, BulkString *zkey_str, BulkString *member, double score){
    ...
    enum RS_STATUS added = _add_member(zset_obj->data, member, score);
    if(added != RS_OK){      // this will atomically add to both hashmap and skiplist
        return RS_ERR;
    }
    return RS_OK;
}

enum RS_STATUS rs_zget(RedisStore *store, BulkString *key_str, BulkString *member_str, double *out){
...
enum RS_STATUS rs_zrange_score_init(RedisStore *store, BulkString *key, double min, double max, RS_ZIterator *out_it) {
...
ZSetMember* rs_ziterator_next(RS_ZIterator *it){
    return sl_next(&it->internal_it);
}

// HELPERS
enum RS_STATUS _add_member(Zset *zset, BulkString *member_str, double score){
    ...
    if(search == HM_OK){    // existing member
        SL_RESULT updated = sl_update(zset->sl, member_str->data, member_str->len, score);
        if(updated != SL_OK){
...
void _free_redis_object(RedisObject *obj){
    if(obj == NULL){ return; }
